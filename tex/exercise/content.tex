% titlepage
\input{content/title}
\input{content/abstract}	\newpage

% table of contents
\tableofcontents
\newpage

% exercises
\input{content/ex_01}		\newpage	% Wirkungsplan
\input{content/ex_02}		\newpage	% Übertragungsfunktion
\input{content/ex_03}		\newpage	% TRF programmieren
\input{content/ex_04}		\clearpage	% Bode P1(s)
\input{content/ex_05}		\newpage	% Nyquist P1(s)
\input{content/ex_06}		\clearpage	% asymp
\input{content/ex_07}		%\newpage	% Blockschaltbild
\input{content/ex_08}		%\newpage	% Steuerung
\input{content/ex_09}		%\newpage	% Regler
\input{content/ex_10}		%\newpage	% offener Regelkreis
\input{content/ex_11}		%\newpage	% Reglertyp
\input{content/ex_12}		%\newpage	% Übertragungsfunktion
\input{content/ex_13}		\clearpage	% Stabilitätsreserve
\input{content/ex_14}		%\newpage	% Geschlossener Regelkreis
\input{content/ex_15}		%\newpage	% Simulink closed loop
\input{content/ex_16}		\newpage	% Endwertsatz
\input{content/ex_17}		\newpage	% Auslegung
\input{content/ex_18}		%\newpage	% Regleridentifikation
\input{content/ex_19}		\clearpage	% Parameter des PI-Reglers
\input{content/ex_20}		%\newpage	% Grenzfall
\input{content/ex_21}		%\newpage	% Stabilität
\input{content/ex_22}		\newpage	% PI-Regler im Simulink
\input{content/ex_23}		\newpage	% stationäre Regeldifferenz PI
\input{content/ex_24}		\newpage	% Signalbegrenzung
\input{content/ex_25}		\newpage	% Integrator
\input{content/ex_26}		\newpage	% Anti-Windup
\input{content/ex_27}		%\newpage	% Vorsteuerung
\input{content/ex_28}		\newpage	% Vorsteuerung-Simulation

\clearpage
\listoffigures

% appendix
\clearpage
\input{content/appendix}
